...
set Items:= {0, 3, 4, 5, 6, 10, 15, 20, 25, 30, 35, 40, 45,nodeE1};
param sizeItems[Items]:= <0> 1, <3> 3, <4> 4, <5> 6, <6> 2, <10> 4, <15>5, <20>3, <25> 3, <30> 1, <35>5, <40>6, <45>5, <nodeE1>11;

param maxL := 10;
param Kf := 16;
param bigM := sum <k> in Items : sizeItems[k];

var x[<i,j> in II] integer  >=0  <= if i == nodeE1 then 0 else 1 end; 
var y[II] >=0 <= Kf; 
var w[Items] binary;

minimize cost : sum <k> in Items : -100 * w[k] + sum <i,j> in II   :  x[i,j]; 
...
subto d01: sum <nodeS1,i> in II : y[nodeS1,i] == (sum<k> in Items : sizeItems[k]*w[k]);
